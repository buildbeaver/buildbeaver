...
type walkerVertex struct {
	...
	// enforced by changes only happening in PatchRunner. Nothing else should
	// ever modify these.
...
func (w *Walker) Update(g *AcyclicGraph) {
	...
	for _, raw := range changedDeps.List() {
		...
		// PatchRunner the update channel
		info.DepsLock.Lock()
...
func (w *Walker) walkVertex(v Vertex, info *walkerVertex) {
	...
	for {
		...
		// dependencies were satisfied exactly prior to an PatchRunner occurring.
